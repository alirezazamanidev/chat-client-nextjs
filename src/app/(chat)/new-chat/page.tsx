...
export default function NewChatPage() {
  ...
  const handleUserClick = (userId: string) => {
    ...
    socket?.emit('createRoom',{receiverId:userId},(chatId:string)=>{
      if(chatId){
        router.push(`/${chatId}`);
      }
    })
  ...
  return (
    <div className="flex flex-col h-full bg-[#17212b] rounded-md overflow-hidden shadow-lg m-2">
      ...
      <div className="px-4 py-3 bg-[#17212b] border-b border-[#0e1621] flex items-center">
        ...
        >
          <svg className="h-6 w-6 text-[#6ab2f2]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            {filteredUsers.map((user:User) => (
              ...
              >
                ...
                <div className="ml-3 flex-1">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-[#64b3f6]">{user.username}</span>
                  </div>
                </div>
              </div>
            ))}
